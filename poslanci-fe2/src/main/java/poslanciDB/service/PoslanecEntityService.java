...
public class PoslanecEntityService extends AbstractService<PoslanecEntity> {
    ...
    {
        return entityManager.createQuery("SELECT P FROM PoslanecEntity P where P.organyByIdObdobi.id = " + idObdobi).setFirstResult(offset).setMaxResults(max).getResultList();
    }
}
